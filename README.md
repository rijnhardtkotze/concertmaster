...
export ANTHROPIC_API_KEY=…  # extract + golden only
...
pnpm run golden                    # prompt regression check, ~2 API calls per case
```
...
|---|---|---|
| `ANTHROPIC_API_KEY` | extract, golden | Only passed to those steps. Never logged, never written to `data/` (writes that match a key pattern throw). |
