...
Until now git was the database: the daily Action committed `data/` and git history held all state. For the rebuild, the Supabase project's Postgres database becomes the single source of truth for listings. Git holds code, content and exports only. For v1 we use Supabase as a managed Postgres (tables, views, constraints, row-level security) and nothing else: no Auth, Storage, Edge Functions or pg_cron.
...
- **The full platform** (Auth, Storage, Edge Functions, pg_cron). Rejected: nothing in v1 needs it.
